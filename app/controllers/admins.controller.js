 ...
 * @return {import("express").Response}
 */
async function qnaDelete(req, res, next) {
  try {
    const id = req.params.id
    await adminsService.qnaDelete(id);
 ...
 * @return {import("express").Response}
 */
async function qnaUpdate(req, res, next) {
  try {
    const data = req.body
    await adminsService.qnaUpdate(data);
...
/**
 * admin 대시보드 배송 통계 그래프 + 요약 정보 데이터 조회
 * @param {import("express").Request} req - 리퀘스트 객체
 * @param {import("express").Response} res - 레스폰스 객체
 * @param {import("express").NextFunction} next - next 객체
 * @return {import("express").Response}
...
async function dashboardStats(req, res, next) {
  try {
    // 서비스에서 통합된 데이터를 가져옵니다.(차트 + 요약 데이터)
    const data = await adminsService.getDashboardStats();

    return res.status(SUCCESS.status).send(createBaseResponse(SUCCESS, data));
  }
  catch (error) {
    return next(error);
  }
